...
const Checklist = (() => {
  ...
  function incrementRevision(topicId, el) {
    revisions[topicId] = ((revisions[topicId] || 0) + 1) % 4;
  ...
  function setDifficulty(topicId, level, starEls) {
    ...
    starEls.forEach((star, i) => star.classList.toggle('active', i < level));
  }

  function updateTopicUI(topicId, row) {
    row.classList.toggle('completed', completed[topicId]);
  }
  ...
  function buildTopicHTML(topic, exam) {
    ...
    const priorityClass = { 'very-high': 'priority-vh', 'high': 'priority-h', 'moderate': 'priority-m', 'low': 'priority-l' }[topic.priority] || 'priority-m';
    const priorityLabel = { 'very-high': '🔴 Very High', 'high': '🟠 High', 'moderate': '🟡 Moderate', 'low': '🟢 Low' }[topic.priority] || topic.priority;

    const stars = [1,2,3].map(n =>
      `<span class="diff-star ${diff >= n ? 'active' : ''}" data-star-id="${id}" data-level="${n}">⭐</span>`
    ).join('');

    return `
      <div class="topic-item ${isDone ? 'completed' : ''}" id="row_${id}" data-topic-id="${id}" data-exam="${exam}">
        ...
        <div class="topic-actions">
          <div class="difficulty-stars" data-difficulty="${id}">${stars}</div>
          <button class="revision-btn" data-rev="${id}">🔁 ${rev > 0 ? rev + '×' : '0×'}</button>
        </div>
      </div>`;
  }
  ...
  function attachListeners(container, exam) {
    ...
    container.querySelectorAll('.topic-item').forEach(row => {
      row.addEventListener('click', (e) => {
        if (e.target.closest('[data-rev]') || e.target.closest('[data-star-id]') || e.target.closest('.topic-checkbox')) return;
  ...
  function resetAll() {
    completed = {}; revisions = {}; difficulty = {};
    save(KEYS.completed, completed);
    save(KEYS.revisions, revisions);
    save(KEYS.difficulty, difficulty);
  }
...
function fireConfetti(targetEl) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    p.style.cssText = `left:${rect.left + Math.random()*rect.width}px;top:${rect.top+window.scrollY}px;background:${colors[Math.floor(Math.random()*colors.length)]};transform:rotate(${Math.random()*360}deg);animation-delay:${Math.random()*0.3}s;animation-duration:${0.6+Math.random()*0.4}s;`;
