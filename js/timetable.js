...
const Timetable = (() => {
  ...
  let schedule = load();
  let modalTarget = null; // { dayIndex, slotIndex }
  ...
  function render() {
    ...
    // Header row
    html += `<div class="tt-header-cell">Time</div>`;
    DAYS.forEach(day => {
    ...
    TIME_SLOTS.forEach((time, slotIdx) => {
      ...
      DAYS.forEach((day, dayIdx) => {
        ...
        if (entry) {
          const colors = SUBJECT_COLORS[entry.subject] || SUBJECT_COLORS['other'];
          html += `
            <div class="tt-slot filled"
                 data-day="${dayIdx}" data-slot="${slotIdx}"
                 style="background:${colors.bg}; border:1px solid ${colors.border}; color:${colors.text};"
                 onclick="Timetable.openEditModal(${dayIdx},${slotIdx})">
              <span class="tt-slot-delete" onclick="event.stopPropagation();Timetable.deleteSlot(${dayIdx},${slotIdx})">✕</span>
              <span class="tt-slot-label">${entry.label}</span>
            </div>`;
        } else {
          html += `
            <div class="tt-slot"
                 data-day="${dayIdx}" data-slot="${slotIdx}"
                 onclick="Timetable.openAddModal(${dayIdx},${slotIdx})"
                 title="Click to add study session">
              <span style="opacity:0.25;font-size:18px;">+</span>
            </div>`;
        }
      });
    });
  ...
  function highlightCurrentSlot() {
    ...
    if (slotIdx >= 0) {
      ...
      if (el && !schedule[`${dayIdx}_${slotIdx}`]) {
  ...
  function saveFromModal() {
    ...
    const key = `${modalTarget.dayIdx}_${modalTarget.slotIdx}`;
    schedule[key] = { label, subject };
    save();
    closeModal();
    render();
  ...
  function clearWeek() {
    if (!confirm('Clear all sessions this week? This cannot be undone.')) return;
    schedule = {};
    save();
    render();
  ...
  return { init, render, openAddModal, openEditModal, deleteSlot, getWeekStats };
})();
