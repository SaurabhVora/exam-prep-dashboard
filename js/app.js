// ============================================================
//  app.js — Main Initializer + Exam Data + Progress Rings
// ============================================================

const EXAM_DATA = {
  ibps: {
    name: 'IBPS SO IT Officer', date: 'August 29, 2026',
    totalQ: 150, totalMarks: 125, duration: '120 min (40 min/section)', negMark: '-0.25 per wrong',
    sections: [
      { id: 'reasoning', name: 'Reasoning Ability', icon: '🧩', questions: 50, marks: 50, time: '40 min', color: '#8b5cf6',
        topics: [
      ...
      { id: 'quant', name: 'Quantitative Aptitude', icon: '📐', questions: 50, marks: 50, time: '40 min', color: '#06b6d4',
        topics: [
      ...
      { id: 'english', name: 'English Language', icon: '📖', questions: 50, marks: 25, time: '40 min', color: '#10b981',
        topics: [
  ...
  ssc: {
    name: 'SSC CGL Tier 1', date: 'Aug–Sep 2026',
    totalQ: 100, totalMarks: 200, duration: '60 min', negMark: '-0.50 per wrong',
    sections: [
      { id: 'gr', name: 'General Intelligence & Reasoning', icon: '🧠', questions: 25, marks: 50, time: '(shared)', color: '#8b5cf6',
        topics: [
      ...
      { id: 'sq', name: 'Quantitative Aptitude', icon: '📊', questions: 25, marks: 50, time: '(shared)', color: '#06b6d4',
        topics: [
      ...
      { id: 'se', name: 'English Comprehension', icon: '✍️', questions: 25, marks: 50, time: '(shared)', color: '#10b981',
        topics: [
      ...
      { id: 'ga', name: 'General Awareness', icon: '🌍', questions: 25, marks: 50, time: '(shared)', color: '#f59e0b',
        topics: [
  ...
  common: [
    { id: 'c1',  name: 'Syllogism',             questions: 'IBPS: 4–5Q | SSC: 1–2Q', priority: 'high' },
    { id: 'c2',  name: 'Coding-Decoding',       questions: 'IBPS: 4–5Q | SSC: 2–4Q', priority: 'high' },
    { id: 'c3',  name: 'Blood Relations',        questions: 'IBPS: 2–3Q | SSC: 1–2Q', priority: 'moderate' },
    { id: 'c4',  name: 'Data Interpretation',   questions: 'IBPS: 15–20Q | SSC: 3–5Q', priority: 'very-high' },
    { id: 'c5',  name: 'Profit & Loss',          questions: 'IBPS Arithmetic | SSC Core', priority: 'very-high' },
    { id: 'c6',  name: 'Time, Speed & Distance', questions: 'IBPS Arithmetic | SSC Core', priority: 'high' },
    { id: 'c7',  name: 'Time & Work',            questions: 'IBPS Arithmetic | SSC Core', priority: 'high' },
    { id: 'c8',  name: 'Ratio & Proportion',     questions: 'IBPS Arithmetic | SSC Core', priority: 'high' },
    { id: 'c9',  name: 'Reading Comprehension',  questions: 'IBPS: 10–15Q | SSC: 5Q', priority: 'very-high' },
    { id: 'c10', name: 'Error Spotting',          questions: 'IBPS: 5Q | SSC: 4–5Q', priority: 'high' },
    { id: 'c11', name: 'Para Jumbles',            questions: 'IBPS: 5Q | SSC: included', priority: 'high' },
    { id: 'c12', name: 'Fill in the Blanks',      questions: 'IBPS: 4–5Q | SSC: included', priority: 'moderate' },
  ],
};
...
function renderAllSections() {
  EXAM_DATA.ibps.sections.forEach(sec => Checklist.renderSection(`ibps_${sec.id}_topics`, sec.topics, `ibps_${sec.id}`));
  EXAM_DATA.ssc.sections.forEach(sec  => Checklist.renderSection(`ssc_${sec.id}_topics`,  sec.topics, `ssc_${sec.id}`));
...
function updateAllProgress() {
  ...
  EXAM_DATA.ibps.sections.forEach(sec => {
    ...
    totalTopics += p.total; totalDone += p.done;
  });
  ...
  totalTopics += cp.total; totalDone += cp.done;
...
function updateStreak() {
  ...
  if (lastVisit === today) {} 
  else if (lastVisit === new Date(Date.now() - 86400000).toDateString()) streak++;
  else streak = 1;
...
function resetAll() {
  if (!confirm('⚠️ Reset ALL progress, timetable, and scores?')) return;
  Checklist.resetAll();
  ['epdash_timetable','epdash_scores','epdash_streak','epdash_pomo_sessions'].forEach(k => localStorage.removeItem(k));
  location.reload();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('resize', () => setTimeout(Scores.renderChart, 100));
});
