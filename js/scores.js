...
const Scores = (() => {
    ...
    if (entries.length === 0) {
      container.innerHTML = `
        <div style="text-align:center;padding:40px;color:var(--text-muted);">
          ...
          <div>No scores logged yet. Add your first mock test result!</div>
        </div>`;
      return;
    }
    ...
    container.innerHTML = [...entries].reverse().map(e => {
      ...
      return `
        <div class="score-entry-row">
          <div class="score-entry-date">📅 ${new Date(e.date).toLocaleDateString('en-IN', { day:'numeric', month:'short', year:'numeric' })}</div>
          <div class="score-entry-exam" style="color:${color}">${examLabel}</div>
          <div style="flex:1;padding:0 12px;">
            <div style="height:6px;background:rgba(255,255,255,0.08);border-radius:3px;overflow:hidden;">
              <div style="height:100%;width:${pct}%;background:${color};border-radius:3px;transition:width 0.8s ease;"></div>
            </div>
          </div>
          <div class="score-entry-score" style="color:${color}">${e.score}/${e.maxScore}</div>
          <div style="font-size:12px;font-weight:700;color:${passColor};padding:3px 10px;background:${passColor}20;border-radius:100px;border:1px solid ${passColor}40;">
            ${pass ? '✅ Pass' : '❌ Below Target'}
          </div>
          <button onclick="Scores.deleteEntry(${e.id})" style="background:none;border:none;cursor:pointer;font-size:16px;color:var(--text-muted);padding:4px;">🗑️</button>
        </div>`;
    }).join('');
  }
  ...
  function renderChart() {
    ...
    if (entries.length < 2) {
      ...
      ctx.fillText('Add at least 2 scores to see the trend chart', W/2, H/2);
      return;
    }
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      const val = Math.round(maxPct - ((maxPct - minPct) * i / 4));
    ...
    drawLine(ibpsEntries, '#8b5cf6');
    drawLine(sscEntries,  '#06b6d4');

    // Legend
    ctx.fillStyle = '#8b5cf6';
    ctx.fillRect(pad.left, H - 20, 14, 3);
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.font = '11px Inter'; ctx.textAlign = 'left';
    ctx.fillText('IBPS SO', pad.left + 18, H - 15);

    ctx.fillStyle = '#06b6d4';
    ctx.fillRect(pad.left + 80, H - 20, 14, 3);
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.fillText('SSC CGL', pad.left + 98, H - 15);
  }
  ...
  function initForm() {
    ...
    form.addEventListener('submit', (e) => {
      ...
      if (isNaN(score) || score < 0 || score > maxScore) {
        showToast(`⚠️ Score must be between 0 and ${maxScore}`); return;
      }
...
const Calculator = (() => {
  ...
  const CUT_OFFS = {
    ibps: {
      overall: { general: 65, pwbd: 55.88 },
      sectional: {
        reasoning: { general: 11.25, pwbd: 6.75 },
        quant:     { general: 10.25, pwbd: 6.75 },
        english:   { general: 10.50, pwbd: 8.00 },
      }
    },
    ssc: {
      overall: { general: 148.27, pwbd: 113.10 },
    }
  };
  ...
  function calculate() {
    ...
    let negMark, marksPerQ, totalMarks;
    ...
    if (scoreEl) {
      scoreEl.textContent = net.toFixed(2);
      scoreEl.style.color = passPwBD ? '#22c55e' : '#ef4444';
    }

    if (statusEl) {
      statusEl.innerHTML = passPwBD
        ? `✅ Clears PwBD cut-off! ${passGen ? '🎉 Also clears General cut-off!' : `⚠️ ${(cutoffs.overall.general - net).toFixed(2)} marks short of General cut-off`}`
        : `❌ Below PwBD cut-off by <strong>${(cutoffs.overall.pwbd - net).toFixed(2)}</strong> marks`;
    }

    if (breakEl) {
      breakEl.innerHTML = `
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value" style="color:#22c55e">+${gross}</div>
          <div class="calc-breakdown-label">Gross Marks</div>
        </div>
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value" style="color:#ef4444">-${deducted.toFixed(2)}</div>
          <div class="calc-breakdown-label">Negative Marks</div>
        </div>
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value">${wrong}</div>
          <div class="calc-breakdown-label">Wrong Answers</div>
        </div>
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value">${pct}%</div>
          <div class="calc-breakdown-label">Score %</div>
        </div>
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value" style="color:#8b5cf6">${cutoffs.overall.pwbd}</div>
          <div class="calc-breakdown-label">PwBD Target</div>
        </div>
        <div class="calc-breakdown-item">
          <div class="calc-breakdown-value" style="color:#06b6d4">${cutoffs.overall.general}</div>
          <div class="calc-breakdown-label">General Target</div>
        </div>`;
    }
  }
